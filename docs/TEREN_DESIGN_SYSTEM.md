...
### 3.11 Navegación por Pestañas (Tabs)
**Propósito:** Alternar entre vistas relacionadas sin perder contexto ni interrumpir el flujo (ej. Tips | Exits | Map en Station Guides).
**Filosofía:** Solo texto, sin iconos decorativos. La jerarquía se comunica mediante el subrayado activo y el contraste tipográfico.

**Especificación Visual:**

| Estado | Color Fondo | Color Texto | Borde Inferior |
|--------|-------------|-------------|----------------|
| Idle | transparent | var(--color-teren-text-muted) | 2px solid transparent |
| Hover | var(--color-teren-interactive-hover) | var(--color-teren-text-main) | 2px solid transparent |
| Active | transparent | var(--color-teren-primary) | 2px solid var(--color-teren-primary) |
| Disabled | transparent | var(--color-teren-text-muted) (opacity 0.5) | 2px solid transparent |

**Reglas de Interacción:**
- Altura mínima: 40px (touch target seguro).
- Gap entre tabs: 4px (evita clics accidentales en móvil).
- Transición: 200ms ease-out en color y borde.
- Disabled: Añadir sufijo visual (soon) en tamaño 10px y peso 400 si el contenido está en desarrollo. Cursor not-allowed.

### 3.12 Badges de Complejidad (Complexity Badges)
**Propósito:** Comunicar nivel de dificultad o severidad en un vistazo (ej. complejidad de estación, riesgo de ruta).
**Diferencia con 3.3:** Mientras 3.3 cubre metadatos neutros, este usa semántica de color para alerta/contexto.
**Paleta Semántica (Alineada a WCAG AA/AAA):**

| Nivel | Fondo Light | Texto Light | Fondo Dark | Texto Dark | Uso |
|-----|-----------|-----------|----------|----------|---|
| **Low** | #D1FAE5 | #065F46 | #064E3B | #6EE7B7 | Seguro, directo|
| **Medium** | #DBEAFE | #1E40AF | #1E3A8A | #93C5FD | Precaución estándar|
| **High** | #FEF3C7 | #92400E | #78350F | #FCD34D | Atención requerida|
| **Extreme** | #FEE2E2 | #991B1B | #7F1D1D | #FCA5A5 | Precaución crítica|

**Reglas:**
- Forma: border-radius: 9999px. Padding: 4px 10px.
- Tipografía: 11px, 700, uppercase, letter-spacing: 0.05em.
- Dark Mode: Los fondos se oscurecen, los textos se aclaran para mantener contraste ≥ 4.5:1.

### 3.13 Tarjetas de Operador y Línea (Operator & Line Cards)
**Propósito:** Mostrar datos jerárquicos de transporte de forma escaneable (Operador → Líneas → Plataformas/Direcciones).
**Principio:** Densidad informativa sin ruido visual. Priorizar la lectura lateral en móvil.
...
<div class="operator-group">
  <h4 class="operator-header">JR East <span class="operator-ja">JR東日本</span></h4>
  <div class="line-row">
    <span class="line-dot" style="background: #9ACD32"></span>
    <span class="line-name">Yamanote Line</span>
    <span class="line-meta">Plat: 4,5 • Dir: Ueno/Shinagawa</span>
  </div>
</div>
<style>
  .operator-group { margin-bottom: 16px; }
  .operator-header { font-size: 13px; font-weight: 600; color: var(--color-teren-text-muted); margin-bottom: 8px; }
  .operator-ja { font-weight: 400; opacity: 0.8; margin-left: 6px; }
  .line-row {
    display: flex; align-items: center; gap: 10px;
    padding: 10px 12px; background: var(--color-teren-surface);
    border: 1px solid var(--color-teren-border); border-radius: 8px;
    margin-bottom: 6px;
  }
  .line-dot { width: 10px; height: 10px; border-radius: 50%; flex-shrink: 0; }
  .line-name { font-weight: 600; font-size: 14px; color: var(--color-teren-text-main); }
  .line-meta { margin-left: auto; font-size: 12px; color: var(--color-teren-text-muted); }
</style>

```
### 3.14 Desglose de Rutas y Transporte (Transport Route Breakdown)
**Propósito:** Mostrar cálculos de tarifas, comparativas de pases y mensajes honestos sin ocultar datos.
**Principio TEREN:** Honesty > Upsell. Si no hay ahorro, se dice claramente. Nunca se fuerza una recomendación.

**Estados del Contenedor:**

| Estado | Borde Izquierdo | Mensaje |
|--------|----------------|--------------|
| **Ahorro Significativo** | 3px solid #22C55E | +¥XX,XXX ahorro estimado |
| **Ahorro Modesto** | 3px solid var(--color-teren-primary) | Ahorro modesto. Evalúa conveniencia. |
| **Sin Ahorro** | 3px solid var(--color-teren-text-muted) | Billetes individuales más económicos. |
...
### 4.2 Transiciones en Pestañas y Contenido Dinámico
- **Cambio de Tab:** No usar animaciones pesadas. El contenido debe aparecer con fade sutil (opacity: 0 → 1 en 150ms). Evitar desplazamientos verticales bruscos.
- **Carga de Datos en Tabs:** Si el contenido tarda >300ms, mostrar skeleton con shimmer en el área de contenido, no spinner bloqueante. El header de tabs siempre permanece interactivo.
- **Expansión de Badges/Desgloses:** Usar slide vertical (height: 0 → auto) con duration: 200ms y easing: cubicOut. Reservar min-height en el contenedor padre para evitar layout shift.
