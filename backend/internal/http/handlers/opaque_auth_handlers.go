...
//
// File naming: parallels opaque_auth.go (the middleware). The three
// pieces — auth.go (JWT), opaque_auth.go (middleware), opaque_auth_handlers.go
// (handlers) — ship side-by-side today and we'll retire both legacy pieces
// together when the cutover flips AUTH_V2_ENABLED.
