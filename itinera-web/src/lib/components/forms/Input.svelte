<script lang="ts">
     ...
     *
     * Visual: a single unified container per TEREN DS 3.10 — the field
     * has no hard borders inside the container; the surrounding card
     * carries the focus halo. Errors are surfaced inline below the
     * input so screen readers can announce them.
     *
    ...
    // Stable id when none is provided — keeps label/for pairing and
    // aria-describedby valid across SSR/CSR. Wrapped in $derived so
    // that updates to `id` (rare but possible in dynamic forms) are
    // picked up by the label/error/help wiring.
...
<div class="flex flex-col gap-1.5">
    <label for={inputId} class="text-xs font-semibold uppercase tracking-wide text-teren-text-muted">
        {label}
    </label>
    <div
