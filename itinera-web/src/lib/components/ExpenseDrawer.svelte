...
{#if isOpen}
  <div class="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/40 backdrop-blur-sm" transition:fade={{ duration: 200 }} onclick={e => e.target === e.currentTarget && onClose()}>
    <div class="bg-teren-background w-full sm:max-w-2xl h-[85vh] sm:h-[80vh] rounded-t-2xl sm:rounded-2xl shadow-2xl flex flex-col overflow-hidden" transition:fly={{ y: 40, duration: 250, easing: cubicOut }} onclick={(e) => e.stopPropagation()}>
  ...
  {#if editingId === exp.id}
    ...
    <div class="p-3 bg-teren-background border-2 border-teren-primary/30 rounded-lg space-y-2">
          ...
          <div class="flex-1 min-w-0">
            <div class="flex justify-between items-baseline">
              <span class="font-bold text-teren-text-main text-base">
                {exp.amount.toFixed(2)} {getCurrencySymbol(exp.currency)}
              </span>
            ...
            {#if exp.notes}
              <p class="text-sm text-teren-text-muted mt-0.5 line-clamp-1 italic opacity-80">
                {exp.notes}
              </p>
            {/if}
          </div>
