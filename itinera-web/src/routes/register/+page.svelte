    ...
    {:else}
      ...
      >
        ...
        <div class="flex flex-col gap-1.5">
          ...
          >
            <!--
              We intentionally DO NOT set aria-label="Mostrar/Ocultar contraseña"
              here. If we did, the accessible-name match by testing-library's
              `getByText(/Contraseña/i)` would collide with this toggle button
              (it matches by accessible name, which includes aria-label).
              The visible text already serves the screen-reader announcement.
            -->
            {showPassword
        ...
        <div class="pt-1">
          ...
          >
            {#snippet children()}
              <!-- Render the parts of the label as plain text via Svelte's
                   automatic escaping. The only markup is a single <a>
                   compiled at build time — no {@html}, no string-based
                   template substitution, no XSS surface. The 'terms'
                   placeholder inside `terms_label` is replaced by an
                   empty string here because we render the link as a
                   real Svelte element right after it. -->
              {$t("auth.register.terms_label", { terms: "" })}
              <a
